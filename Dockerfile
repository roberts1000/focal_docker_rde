...
ENV DEFAULT_NODEJS_VERSION=$NODEJS_MAJOR_VERSION.12.1
...
RUN sudo chown $DEV_USER:$DEV_USER $BUILD_TMP_DIR/chrome && \
  ...
  sudo apt-get update -qq && \
  sudo apt-get install -y libasound2 fonts-liberation libappindicator3-1 libnss3 libnspr4 libxss1 libu2f-udev xdg-utils && \
...
RUN sudo apt-get -y autoremove && \
  ...
  rm $BINARIES_DIR/chromedriver_linux64.zip && \
...
CMD sudo redis-server $REDIS_CONFIG_FILE && \
  ...
  # Start MySQL server
  sudo service mysql start && \
  # Start x-font server so XMing can have better fonts.
  sudo xfstt --daemon && \
  /bin/bash --login
# *****************************************************************************************************************************
